...
exit /B 1)
"%~dp0packages\NUnit.Runners.2.6.2\tools\nunit-console.exe" /exclude=%excluded%,Errors,AnswerOptions,ObjectivesList,ExperiencesList,QuestionsList,Question /labels /out="%~dp0TestsResults\TestResult.txt" /xml="%~dp0TestsResults\TestResult.xml" "%webSiteDir%easygenerator.AcceptanceTests.dll"
