...
Scenario: Button CreateAndEdit is disabled if title text is empty on create objective view
When open page by url 'http://localhost:5656/#objective/create'
...
Scenario: Button CreateAndEdit is enabled if title text is not empty on create objective view
When open page by url 'http://localhost:5656/#objective/create'
...
Then page does not contain element with text 'Objective2'
When refresh page
Then objectives tiles list contains only items with data
| Title      |
| Objective1 |
