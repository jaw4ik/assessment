...
namespace easygenerator.Web.Controllers
{
    public class FileStorageController : Controller
    {
        ...
        public static readonly string GetFileApiUrl = "filestorage/";
        ...
        public ActionResult Upload()
        {
            ...
            {
                return new JsonErrorResult("Input data is invalid");
            }

            var file = Request.Files.Get(0);
            if (file.ContentLength == 0 || string.IsNullOrEmpty(file.FileName))
            {
                return new JsonErrorResult("File data is empty");
            }

            if (file.ContentLength > MaximumFileSize)
            {
                return new JsonErrorResult("File size too big");
            }

            var fileInfo = new FileInfo(file.FileName);
            if (!AllowedExtensions.Contains(fileInfo.Extension.ToLower()))
            {
                return new JsonErrorResult("Forbidden file extension *" + fileInfo.Extension);
            }
            ...
            return new JsonSuccessResult(new { url = Path.Combine(Request.UrlReferrer.AbsoluteUri, GetFileApiUrl, newFileName + fileInfo.Extension) });
        }
